...
**Current status:** released through v0.1.25; the roadmap is fully implemented — P2 minimal closed loop, P3 health/graph family, P4 optional host layer (Web + MCP + IM, through P4.17's Streamable HTTP transport, P4.18's move to MCP SDK v2 / protocol `2026-07-28`, P4.19's read-only Web panel for the *opposite* direction — the external MCP servers agentao already injects into this KB — P4.20's in-browser rendering of ` ```flint ` chart-spec blocks into data charts, P4.21's IM host: a platform-agnostic core plus thin per-platform adapters, v1 = personal WeChat + Feishu, and P4.22's clickable citations: a zero-LLM `/page` command everywhere plus Feishu card callbacks that a click turns into a *synthesized inbound message*, so all five intake gates are reused and the Q&A path needed zero changes), P5 retrieval + multi-format ingest, and every half-phase P2.1–P5.4. No roadmap spec remains unimplemented; per-phase flags, behavior, and red lines live in the `docs/P*.md` file for that phase, not here. P4.22's main path passed real-machine acceptance on 2026-08-17 (click → toast → the page comes back), but several items are still open — which WS frame type Feishu delivers card callbacks on (the code handles both, so the shim stays), the unauthorized-click and forwarded-card paths — so read `docs/P4.22-IM可点引用.md` §9 for what was actually observed before claiming any individual item is verified. P4.23 then moved the runtime floor to `agentao[cli]>=0.5.3,<0.6` (first upper bound — 0.5.0 proved this project's minors carry breaking changes) and closed the gap behind a fact worth internalizing before touching the embedded path: **`arun` returning normally does not mean the model answered.** agentao returns `[LLM API error: …]`, empty-response placeholders, truncation and loop-termination notices as *ordinary strings*; the host reads `agent.last_turn` to tell them apart, and an incomplete turn is a **structured return, never an exception** (raising would make the goal loop's consecutive-no-progress counter unreachable). See `docs/P4.23-Agentao0.5.3接入.md`.
