...
**Current status:** released through v0.1.16. Implemented = P2 minimal closed loop + P3 health/graph family + P4 optional host layer (Web + MCP) + P5 retrieval/multi-format ingest, including every half-phase P2.1–P5.4, plus the P4.13/P4.14 pure-frontend rich-render half-phases (Web in-browser mermaid diagrams; KaTeX+mhchem math/chemistry + highlight.js code highlighting — both vendored, server-zero-change, fail-keep-source) and P4.15 (Web tool-confirm / `ask_user` human-in-the-loop: workspace-write ASK decisions are confirmed in the browser instead of silently auto-approved, with hard timeout + cancel + disconnect-releases-lock; `--confirm {ask,auto}`) and P4.16 (Web long-task goal / auto-continuation `/goal`: a host-owned `while goal.is_active` loop drives multi-turn progress toward an objective until the agent self-reports complete/blocked or a time/turn budget trips — **reusing** agentao's shipped `cli.goal_state.GoalState` / `cli.duration.parse_duration` / `tools.goal.UpdateGoalTool` primitives (needs `agentao[cli]>=0.4.13`), guanlan adds only the Web orchestration layer: per-conversation `.agentao/goals/<id>.json` sidecar, SSE continuation driver, `/goal{,/budget,/pause,/resume,/edit,/clear,/run}` endpoints, `--goal {on,off}`; `--turns` (outer continuation rounds) is orthogonal to `max_iterations` (inner per-turn tool loop) — do not conflate) and P4.17 (MCP remote transport: `guanlan mcp --transport http` adds the official **Streamable HTTP** transport beside stdio — **same read-only tool set, only the transport changes**: `tools.py` unchanged, `build_mcp` gains only an `allow_ask` registration gate, all transport/bind/auth wiring lives in `serve_mcp`; **safe defaults** — still binds `127.0.0.1` (决策P4-4 red line), non-loopback bind requires a bearer token (read from an env var, never CLI plaintext / never on disk; minimal constant-time middleware, no OAuth) or refuses to start, `ask` over http is off by default (`allow_ask=(transport=='stdio' or --allow-ask)`: stdio always 7 tools, http default 6, `--allow-ask` → 7), stateless + DNS-rebinding protection, TLS external; full OAuth (MCP 2025-06-18) / multi-tenant source-level scoping deferred to E2; zero dependency change — the installed mcp SDK already ships uvicorn/starlette; [`docs/P4.17-MCP远程传输.md`](docs/P4.17-MCP远程传输.md)); no roadmap spec remains unimplemented.
...
(`ingest` / `query` and the Web host's chat drive Agentao + the skill and need a configured model; `init` / `check` / `health` / `lint` / `graph` / `reindex` are the zero-LLM ones runnable offline (`convert` is also zero-LLM from guanlan's side, but shells out to the `pdf-to-markdown` skill's external backends — MinerU/marker/pypdf — so a real conversion needs at least one installed). `guanlan web` needs the optional `web` extra: `uv pip install 'fastapi>=0.110' 'uvicorn>=0.29' 'markdown>=3'`, or `pip install 'guanlan-wiki[web]'`. `guanlan mcp` needs the optional `mcp` extra: `pip install 'guanlan-wiki[mcp]'` — a read-only MCP **server** over stdio (default) or Streamable HTTP (`--transport http`, P4.17), distinct from DESIGN's reverse-direction "Tool 注入" where Agentao is the MCP *client*.)
