...
**Current status:** released through v0.1.22; the roadmap is fully implemented — P2 minimal closed loop, P3 health/graph family, P4 optional host layer (Web + MCP + IM, through P4.17's Streamable HTTP transport, P4.18's move to MCP SDK v2 / protocol `2026-07-28`, P4.19's read-only Web panel for the *opposite* direction — the external MCP servers agentao already injects into this KB — P4.20's in-browser rendering of ` ```flint ` chart-spec blocks into data charts, and P4.21's IM host: a platform-agnostic core plus thin per-platform adapters, v1 = personal WeChat + Feishu), P5 retrieval + multi-format ingest, and every half-phase P2.1–P5.4. No roadmap spec remains unimplemented; per-phase flags, behavior, and red lines live in the `docs/P*.md` file for that phase, not here.
