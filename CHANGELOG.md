...
- **底座升到 agentao 0.5.3，并补上"模型到底有没有答出来"这道判定**（P4.23，见
  [`docs/P4.23-Agentao0.5.3接入.md`](docs/P4.23-Agentao0.5.3接入.md)）。

  起点是一件一直在发生、但宿主看不见的事：**`arun` 正常返回不等于模型答出来了**。agentao 会把
  `[LLM API error: …]`、空响应占位、截断与循环终止提示当**普通字符串**返回，不抛异常。于是只看
  "没抛异常"的宿主会把这些原样当成知识库的回答发出去——Web 的 done 帧、IM 的回复都如此，用户
  只会以为知识库疯了。判定口径取 agentao 公开的 `agent.last_turn`。

  - **未完成是结构化返回，不是异常。** 一抛就落进 `run_goal` 的通用 `except`，"连续无进展才
    blocked"的计数永远攒不到阈值——上游能计数，正因为这些结果是正常返回、返回后才读 `last_turn`。
  - **判定必须验类型才敢信。** `MagicMock` 对任何属性都答真值；不逐字段验类型就会把每一轮都读成
    成功或失败，全看运气。读不到可信读数就整体降级回旧行为，绝不误报失败。`is_answer` 按公开
    公式自算而不读属性，同理。
  - **失败轮契约**：轮内真异常也累计墙钟、也出 `turn_done`。原先这条路既不记账也不出帧——"已写
    文件、随后 API 失败"时 check 结果与撤销 token 明明已生成却随帧一起丢掉，用户拿不到撤销入口；
    墙钟蒸发还让 `--for` 预算永不 trip。**落盘自身失败仍只记日志**：磁盘满不该中断一次本来成功
    的问答，那是既有降级契约，未改。
  - **goal 无进展护栏**：连续 3 轮无进展收为 blocked，`blocked_by="host"` 与 agent 自陈的 blocked
    分开呈现（否则把"模型连 3 轮没出声"误报成"它在等你补充信息"）。调过工具算有进展，**但
    `llm_error` 不吃这条豁免**——`tool_count` 按轮累计，一次成功调用会让此后每轮都重置连击，
    provider 挂了就永远空转，而那正是这条护栏存在的理由。
  - **恢复会话时清掉模型专属推理数据**：观澜的 restore **有意不回放盘上的 model**、一律绑当前
    进程模型，所以"换模型恢复"是常态而非边角——不清，下一次 LLM 调用就会被 provider 拒掉。直接
    用上游的清理函数而**不是**自己抄一份字段清单：上游新增一种 wire 只需加一项，抄来的副本会
    静默漏掉它，而**漏清理是沉默的**（正向用例全绿，只有真换模型时才炸），故另配一条逐项断言
    `WIRE_CARRIER_KEYS` 的契约用例。
  - **关掉后台子 Agent**（`bg_store=None`）：后台子任务能活过父轮收尾，那时写日志已 `end_journal`、
    写锁已释放，它的写拿不到撤销 token、混进下一轮日志、并脱离本轮 check 范围。**但它只关后台**
    ——前台子 Agent 照常注册，故工具继承另行解决：`guanlan_search` 声明 `copies_to_subagents`
    （0.4.24 起不声明就进不了子 Agent，而 skill 与 `query.py` 都写着"先用可用的 search 入口召回"，
    子 Agent 读得到指令却没工具就是死指令）。
  - **新增 `tests/test_agentao_contract.py`：用真 factory、真 `Agentao`** 跑契约（LLM 端点指向死
    地址，构造期不发请求，离线可跑）。既有 Web 用例全把 factory 猴补成替身——替身永远符合我们
    记忆中的旧签名，**0.5.0 那批破坏性改动在全替身的测试库里可以一条都测不出来**。
...
- **`guanlan im --log-level`：把早就埋好的 DEBUG 放出来**（SAG 2026-09 反向评审 §2.B，见
  [`docs/P4.21.1-IM日志出口.md`](docs/P4.21.1-IM日志出口.md)）——`guanlan/im/` 一路埋着 DEBUG，
  埋的正是真机排障要问的那几件事（卡片回调走的是 CARD 还是 EVENT 帧、消息被哪道闸丢的），但宿主
  从不配置 `logging`，于是 INFO/DEBUG 没有任何 handler 接（`lastResort` 只兜 WARNING+），这些记录
  默认一个字也不出现。想看它们，此前唯一的办法是**改源码插 print 再跑一次真机会话**——而真机会话
  要人拿着手机点。**埋点早就写好了，缺的只是出口。**

  - **作用域即安全边界**：handler 只挂 `guanlan` 这棵 logger 树、级别也只设在它上面，**绝不碰
    root**。于是 `--log-level debug` **在结构上不可能**打开 `lark_oapi` / `httpx` 的 DEBUG（那会把
    **消息正文**打出来）。**用作用域代替黑名单**——黑名单要逐个列 logger 名，漏一个就把用户私聊
    写进日志文件或 journal。副作用是好的：不碰 root 意味着第三方 WARNING+ 仍照旧走 `lastResort`，
    与本次改动之前逐字节相同。
  - **默认档 `warning` 不改变"看得见什么"**，只改变"长什么样"（多了时刻、级别与 logger 名——
    长驻宿主排障要靠时刻把"我点了按钮"那一秒和日志行对上）。
  - 安装点在 `serve_im` 最前面、早于每一道闸（拒启恰恰是最需要日志的时刻），有专测用"白名单
    全空拒启"当探针钉住这个顺序。
  - **只给 stderr，不给 `--diag-log <file>`**：落文件是另一档承诺（轮转、留存、误传），真机排障
    有 `2>im.log` 就够。也**不复用 `mcpdiag` 的脱敏器**——那是为 MCP 配置里的已知字段写的，与
    任意文本不同构；姿态是**不打消息正文**，而不是"打了再遮"。
...
- **检索反链先验权重 0.5 → 0.05：原值实测是净质量回退**（gbrain v0.50 反向评审 §1.1，收据见
  ...
  按构造饱和（每条黄金 query 的正确页都靠标题精确命中赢、三条聚合断言恒等于 1.0），**W 取 0 还是 2 都绿**。
  补做的扩样扫描：三个本地真实库（未入仓）× 三探针（标题 / 正文逐字片段 / 长句，各有客观正确页）×
  五取值，n=1720，跑产线 `score()`——微平均 P@1 `W=0` .7837 / `0.05` .7855 / `0.1` .7860 / `0.25` .7471 /
  **`0.5` .6988**：**原值在 9 组探针里没有一组优于关闭**，微平均掉 8.5 个百分点，最差一组配对 **+16/−79**。
  典型形状就是文档先验越权——入链 4 的页被入链 19 的邻近页顶掉；正文里**逐字抄**的片段，源页被入链更多
  的兄弟页顶掉。

  - **不是关掉了事**：boost 确实能把强枢纽页从 top-10 之外捞回 rank-1（一组净 +11），所以取最优区间
    （0.05–0.1）的**下沿 0.05**——与 0.1 净收益相同（差 1 条 query），但逐 query 扰动明显更小
    （掉出 rank-1 每组 ≤4，0.1 在两个库分别掉 13 / 9）。文档先验本该是"同分微调"，不该改写名次。
  - **用户可见的只有名次**：同库同 query 的排序更贴近纯 BM25。**召回集不变**（收录门槛仍判在 boost
    之前，决策P5.3-7 不破）、字段 / JSON 契约 / 退出码不变、四处接入点（CLI·Web·MCP·chat）仍同名次。
  - **连带反转一条测试**：`test_backlink_rerank_lifts_hub` 原断言要求纯 BM25 落后 **30%** 的枢纽页
    （1.5100，5 入链）排到分高者（1.9676，0 入链）之前——那不是 P5.3 §1 写的"同分时上浮"，正是本次
    实测为有害的那条行为，**等于给它发合格证**。已整条反转为
    `test_backlink_prior_is_applied_but_does_not_overturn_relevance`：① 因子逐字可验（读模块现值、
    不写死 W）；② 分差 30% 的枢纽页不得越过分高者。
  - **补上黄金集一直缺的困难档 `HARD`**：正确页**只在正文命中**（标题与别名都不含 query 词，故拿不到
    P5.0 字段加权），诱饵是 8 入链的枢纽页——这条 query 的名次**真的取决于 W**，而 `GOLDEN` 那 14 条
    做不到（正确页都靠标题精确命中以大比分赢，W 取 0 还是 2 都绿）。用例**自证有牙**：算出这对页的
    翻盘阈值 `w_flip ≈ 0.315` 后两头断言——现值须在其下（此刻绿得有意义）、**旧默认值 0.5 须在其上**
    （即它确实挡得住这次真实回归），防止语料日后被改软、护栏退化成永远绿的装饰。
  - **决策P5.3-2 改口**：W 的取值**必须带收据**，再动须重跑 §4.5 的扫描并更新那张表，不接受凭直觉调参。
    §4.5 同时写明收据的四条限度（代理指标非用户研究 / 三库皆 CJK 专业领域 / 小库名次本就敏感 /
    扫描脚本依赖本地私有库，不入仓不进 CI）。
...
- **`wiki/` 整个没了的时候，报告还在说"改动已留在磁盘"**（SAG 2026-09 反向评审 §3.A）——
  `report_outcome` 在 check 失败时固定打这一句，而 `wiki.missing` 下**目录都没了、没有任何改动留在
  任何地方**，且这句话恰好出现在用户最慌的时刻。现在按违规类型分岔，`wiki.missing` 下改说「wiki/
  已不存在或不是目录，没有改动留在磁盘上。`raw/` 完好（快照门禁先于本检查核对过），生成层可从源
  重建」。**主动给出 `raw/` 完好这条事实**：门禁先判 raw 再跑 check，能走到 check 失败就意味着 raw
  快照 diff 为空——生成层没了的时候，这是唯一还能让人松口气的事。两条用例钉死分岔两侧。

  **只改文案、不改自愈分类**：原本想把"本轮新发生的整个目录丢失"划进直接判死档，复核后放弃——
  `raw/` 全程有快照门禁守着，agent 破坏不了唯一真相；而真正的损失在自愈轮之前就已经发生，自愈轮
  多烧零轮还是两轮相对于"整个知识层没了"是舍入误差。理由与翻案条件记在笔记 §3.A。

- **`raw/ 被改动` 的文案把因归在了 agent 头上**（SAG 2026-09 反向评审 §2.C）——门禁只比对前后两次
  快照，**分不清改动出自本轮 agent 还是另一个进程**。而观澜的写协调是**进程级**的（Web 宿主的单写者
  队列 + `workers=1`），跨进程无锁（P2 §11 把并发锁明确推后、P4 §4.2 重申）。于是同时开着
  `guanlan web` 或另一个终端在写时，这条**最严重的红线**会如实报出改动、却让人去审一个没越界的
  agent。现在多一行：「若同时开着 `guanlan web` 或另一个终端在写本库，改动也可能来自那一侧——门禁
  只比对前后快照，分不清是谁改的」。事实照报、处置指引不变，有专测钉死三者共存。

  **刻意只改文案、不加跨进程锁**：触发需要同一个人同时开两个写操作，主导失败模式是响的不是哑的，
  至今零实际发生记录；而锁的真实成本是八个写命令的入口盘点、`convert --ingest` 那类嵌套的重入处理、
  以及 Windows 无 `flock`。翻案的触发条件写在笔记 §2.C：出现一个**不由用户逐次发起**的写者。
