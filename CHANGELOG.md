...
- **`guanlan grep`：第二条召回路，按**原文字面**找**（SAG 2026-09 反向评审 §1.D，见
  [`docs/P5.5-字面检索.md`](docs/P5.5-字面检索.md)）——`search` 的分词把非 CJK 段切成 `[a-z0-9]+`，
  于是 `bge-m3` 成 `bge`+`m3`、`P4.22` 成 `p4`+`22`、`v0.1.23` 成三段。召回不丢（两侧同一
  `tokenize`），丢的是**精度**：一条标识符查询实际退化成"其中最稀有的那一段"，数字段在真实库里是
  高频噪声、被 IDF 压到近乎无贡献。而观澜的语料恰恰塞满这类标识符——决策编号、阶段号、版本号、
  模型名、slug。实测两页微库里 `search bge-m3` 会把一张**根本没有 `bge-m3`**、只分别含 `bge` 与
  `m3` 的页以 0.39 分召回。

  - **打印 `页:行号` + 片段**，照着能直接跳到那一行。行号按**原文件**计（frontmatter 不参与匹配
    但计入行号），否则带 frontmatter 的页整体偏移若干行、跳过去是错行。
  - **口径钉死、无旋钮**：`re.escape` 后字面匹配（输入**永不当正则**）+ `IGNORECASE` 直接在原行上
    跑（故偏移对原文有效，不走"先 lower 再 find"那条会静默错位的路）；**不做 NFKC / 全半角归一**
    ——归一过的"命中"在文件里并不字面存在，会让"页:行号 + 片段"这条证据链失真。
  - **双封顶且触顶必说**：单页至多 5 条、全库至多 `--limit`（默认 50），触顶 `truncated=True`；
    反向也钉死——没触顶不许报、恰好等于上限不报。截断只截命中列表，`pages_searched` 照实数全。
  - **不动 `tokenize` 一个字节、不碰 BM25、不影响任何既有黄金集**：它是新增的第二条路，不是改动
    第一条路的口径。为什么不改分词见 P5.5 §0——扩展词降权根本不是分词改动，观澜的
    `tokenize → list[str] → tf` 不携带词项来源。
  - 不建索引、不落派生物；不扫 `raw/`；暂不接 Web/MCP（内核与 `grep_result_dict` 已按可复用写好，
    仿 P5.0 → P5.1 另作半阶段）。
