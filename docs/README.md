...
| | |
|---|---|
| 📖 [用户指南 `guide/`](guide/) | 安装、上手、各命令、Web/MCP/IM 宿主(中英双语) |
...
- [P3.7-语义审计](P3.7-语义审计.md) —— `audit`:漂移源确定性粗筛 + LLM 复核过期论断(走门禁)
- [P3.8-链接归一](P3.8-链接归一.md) —— `[[wikilink]]` 解析期消变体(零正文改写)
- [P3.9-源撤回](P3.9-源撤回.md) —— `remove`:人发起把误摄/已撤稿源移入 `.trash/`(零 LLM)
- [P3.10-页型目录一致性](P3.10-页型目录一致性.md) —— `health` 页型↔目录漂移 advisory
...
- [P4.23-Agentao0.5.3接入调研](P4.23-Agentao0.5.3接入调研.md) —— **调研,未实施**:底座 0.4.17 → 0.5.3 沿用现有子进程 + 嵌入架构(无需迁 ACP);随升级补 `last_turn` 结果判定与会话恢复时的推理数据清理
...
- [P5.3-检索backlink重排](P5.3-检索backlink重排.md) —— 确定性入链先验对 BM25 召回温和重排
