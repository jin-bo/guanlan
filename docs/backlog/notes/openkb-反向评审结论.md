...
- **PageIndex 长文树检索** 🅴→**已重开细化，见 §9**：哲学极贴合（"vectorless、无向量库"= P5.0 立场）。原判"E1 量级、现在别动"过于笼统——深挖后**可拆**：引 PageIndex **依赖**仍 🔴 别借（拖 litellm/openai-agents），但其**确定性树结构**这一半可在 guanlan 既有 markdown `##` 标题上原生重建（零-LLM、无新依赖），是攻 guanlan"单文档深度"弱轴的 🟡 候选。详见 **§9**。
...
## 9. 增量评审：PageIndex 长文树检索深挖（重开 §7，2026-06-19）

> 触发：上游线索把 guanlan 的检索弱点框定为**"单文档深度"轴**（OpenKB/PageIndex 的强项，gbrain/SAG 是"集合基数"轴、Postgres 路线）——这是 §7 那条笼统 🅴 当初没有的视角。深挖 OpenKB 0.3.0.dev1 对 PageIndex 的集成后，原判**可拆**，结论细化如下。

### 9.1 证据级分界（PageIndex 到底哪部分零-LLM、哪部分必 LLM）

挖 `openkb/indexer.py` / `tree_renderer.py` / `agent/{compiler,query,tools}.py` + `pyproject.toml`（`pageindex==0.3.0.dev1`）：

| 层 | 生成者 | 确定性？ |
|---|---|---|
| **树结构**（章节/页层级节点） | PageIndex 库，**基于 TOC/标题/页层级自动检测**（非语义分段） | ✅ **确定性、零-LLM** |
| 节点 summary | PageIndex 库**内部 LLM**（依赖拖 litellm/openai-agents） | ❌ LLM |
| 文档 overview / concept·entity 页 | OpenKB agent LLM | ❌ LLM |
| **取页内容** `get_wiki_page_content(doc, pages)`（`tools.py:91-132`） | 读 `wiki/sources/{doc}.json` 按页码返回 | ✅ **确定性、零-LLM、无向量** |
| **"reasoning-based retrieval"** = 选哪几页（`query.py:37-39`） | **LLM 读树的文本大纲、自己推理出页码范围**（每 query 一次 LLM 调用；**不是**算法在树上导航） | ❌ LLM |

- **关键澄清**：所谓 vectorless"推理式检索"= **LLM 读「目录大纲」挑章节/页码 → 确定性工具取那几页**。检索单位是**页码/章节**，不是整文档。无向量库、无 embedding（这正是哲学贴合处）。
- **依赖红线**：引 `pageindex` 包 = 引 litellm/openai-agents（PageIndex 自带 LLM 客户端）——直接撞观澜"脚本不自带 litellm/自管密钥、LLM 只经 agentao"（DESIGN line 262）。**故 PageIndex 这个库本身 🔴 别引**（原 §7 E1 判定对**依赖**成立）。
- 本地默认/云可选：本地模式无需 `PAGEINDEX_API_KEY`（云模式加 OCR），但本地仍需那串 LLM 依赖出节点 summary。

### 9.2 真正可借的是**形状**，且 guanlan 能原生零-LLM 拿到、无需 PageIndex

- **观澜的弱轴**：P5.0 检索以**整页为单元**、BM25 整页正文召回；长 source/synthesis 页只能整页返回，**无页内章节粒度的命中/引用**。且 `[[wikilink]]` 解析期 `link_stem` 把 `#anchor` **剥掉**（`check.py`，锚点被忽略）——**页内 `#section` 引用当前不解析**，正是深度轴的结构盲区。
- **红线相容的净新增**：PageIndex 的**确定性树结构**对观澜不必引库——**观澜 source/synthesis 页本就是带 `##` 标题的 markdown**。可零-LLM 从**标题层级**建一棵**页内大纲树**（章节 + 锚点），作**可从 markdown 幂等重建的派生物**（守"markdown 唯一真相"），让检索/引用落到**章节粒度**而非整页；`[[page#section]]` 锚点解析顺带可解。**取章节**是确定性工具（对应 OpenKB `get_wiki_page_content` 那半）。
- **LLM 那半落 query 道**：让 query 的 Agent 拿到页内大纲、**推理选章节再读**（对应 OpenKB `query.py` 流）——这是 query 的 LLM 道增强（经 Agentao），**不需要** PageIndex、不进零-LLM `search`。与 SAG Standard 模式同性质，park 为 query 工作法选项。

### 9.3 结论细化（替代 §7 那条笼统 🅴）

- 🔴 **引 PageIndex 库 / 云 API** —— 别借（拖 litellm/openai-agents 撞 LLM 归口红线；云撞 agentao 绑定）。原 §7 E1 判定对**依赖**成立。
- 🟡 **原生 markdown 标题大纲树 + 章节粒度检索/引用**（零-LLM 派生物可重建、`[[page#section]]` 锚点解析、取章节确定性工具）—— **红线干净的净新增**，攻"单文档深度"弱轴，**无新依赖、无向量库**。候选并入 [`cjk-retrieval-enhancements.md`](cjk-retrieval-enhancements.md) 检索增强线 / `docs/P5.x`。
  - ⚠️ **urgency 实证驱动、别冒进**：观澜**摄入即蒸馏**——`wiki/sources/` 存的是**摘要页**不是长文页码数组（异于 OpenKB 存长文 JSON 页数组），故"长页稀释检索"在观澜本就被设计软化。**触发条件**=真实库出现长 synthesis/concept 页让整页 BM25 召回变粗时，才值得做（同其它 P5 增强的实证门槛）。
  - ⚠️ **须钉死**：大纲树是**派生物**（从 `##` 标题幂等重建、byte-stable、不落权威）；章节锚点解析触 `link_stem`/`graph.broken≡check` 归口须同口径 + 撞名守护（承 P3.8）；**不改集合基数天花板**（这是深度轴，几千页的集合上限不变，见 gbrain freshness 那条的规模讨论）。
- 🔵 **佐证**：OpenKB+PageIndex 实证了**长文档检索深度可纯结构化（TOC/标题树）+ LLM 读大纲达成、无需向量库/DB**——从同类（Python·markdown·无 DB）项目侧，**背书观澜在不破"无向量库/markdown 唯一真相"红线下攻深度轴**是可行的。比 gbrain/SAG 的 Postgres 路线对观澜直接得多。记入 DESIGN §8 检索增强。

**一句话**：§7 笼统的"PageIndex=E1 别借"应拆成——**库别引（🔴 依赖拖 LLM）**，但**它的确定性标题树形状可在 guanlan 既有 markdown 上原生零-LLM 重建（🟡 候选，攻单文档深度弱轴，无依赖无向量库）**，且 OpenKB 实证了这条路对"无 DB markdown"项目可行（🔵 佐证）。urgency 实证门槛：等真实库出现长页稀释整页检索。
