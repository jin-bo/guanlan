...
- **Data charts**: ` ```flint ` fenced blocks ([flint](https://github.com/microsoft/flint-chart) chart-spec JSON) → bar / line / scatter / heatmap and 33 other chart types (P4.20)

**About data charts**: the data is **inlined whole** in that markdown block (the renderer accepts **only inline `data.values` and rejects `data.url`** — the browser never goes online to fetch data), so a chart is 100% reconstructible from that block alone, and `check`/`graph`/`search` keep working unchanged. It is a **snapshot of the moment the data was pulled** and never refreshes itself; to update it, ask again and produce a new block. Charts are **not interactive drill-downs** (read-only presentation, not a BI panel). The three failure states each mean something different: "data too large" → fewer rows, "data truncated" → fewer **categories**, "render failed" → fix the spec. To let the Agent actually write valid blocks, install the bundled `flint-chart-author` skill (see [CLI commands · install-skill](03-cli-commands.md)).

The renderers are all **vendored, bundled, non-CDN, offline-capable**, and **lazy-loaded** (pages without such content load nothing); any load/syntax failure **keeps the source** and never blanks the page. Security-wise: KaTeX `trust:false` (blocks `\href`/`\html*`), mermaid `securityLevel:'strict'`, highlight is fed escaped text, and flint goes through an entry allowlist + key-domain exit assertion + SVG rendering + a tooltip path that never touches HTML — **products are not assumed unconditionally trusted**; trust boundaries are in the per-phase design docs. CLI / MCP text channels do not render (they return literal source).
...
See: repo [`docs/P4-Web宿主.md`](../../P4-Web宿主.md) and the `P4.x` docs ([P4.1](../../P4.1-Web投喂.md) / [P4.5](../../P4.5-可写Web工作会话.md) / [P4.6](../../P4.6-Web上传与晋级.md) / [P4.9](../../P4.9-只读多会话.md) / [P4.13](../../P4.13-Web-mermaid渲染.md) / [P4.14](../../P4.14-Web数学化学代码渲染.md) / [P4.15](../../P4.15-Web工具确认.md) / [P4.19](../../P4.19-Web-MCP诊断.md) / [P4.20](../../P4.20-Web-flint图表渲染.md), etc.).
