...
- **Records which backend parsed it** (decision P5.2-13): landing a source adds a `parsed_by` frontmatter key holding the backend that **actually ran** (`mineru` / `marker` / `python`). `--backend auto` is a tiered fallback whose three tiers differ by an order of magnitude in quality, and since `raw/` is immutable and the wiki layer grows out of it, without this there is no field that can answer "which pages in this base were built on degraded text". It sits alongside `origin` (which file it came from) as provenance: one records where from, the other how. Only the final backend is recorded (not the fallback chain), and if no backend marker can be read the key is simply omitted. The Web "parse → promote" path records it too, and it survives promotion.
