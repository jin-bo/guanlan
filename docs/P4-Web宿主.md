...
| `guanlan/web/chat.py` | **问答 / 多轮会话**：只读进程内嵌入 `Agentao`（`build_from_environment` + 构造期 `transport=build_compat_transport(...)` + `arun`），一会话一对象 + `asyncio.Lock`，transport 固定回调→当前 turn emit→`token` 流；**仅内存、v1 不落盘** | §4.4 **\[决策P4-8]** |
...
| `guanlan/web/static/index.html` | 单页前端（vanilla JS + `fetch`，无 npm/无构建/无 CDN）；侧栏页面树 + 主区单页 + 对话区（chat）+ 顶栏动作（ingest/check/health/lint/graph） | §6 **\[决策P4-3/P4-9]** |
...
> 复用面（只 import、不复制逻辑）：**写**路径 `ingest.run_ingest`；**零 LLM** `check.run_check` / `health.run_health` / `lint.run_lint` / `graph.build_graph` · `graph.graph_entrypoint` / `pages.{iter_pages, load_page, link_stem, WIKILINK_RE}` / `paths.require_kb_root` / `errors.*`。**问答**路径不复用 `query.run_query`，改走 `chat.py` 的 `agentao.embedding` 嵌入（§4.4）。
...
- **P2 §4.3 嵌入四坑逐一化解**：① 构造凭据缺失 → 用 `build_from_environment()`（读 `.env`/`~/.agentao`），不裸构造；② skill 不自动激活 → 构造后显式激活 `guanlan-wiki`；③ `<wd>/agentao.log` 污染 → 传我们自己的 `logger=`，不落库内；④ 内部 API 耦合 → 只用 `build_from_environment`/`.arun`/一个文本回调这一组**嵌入契约**面。
...
- **意象落到组件**：背景 `--lan-paper`，可叠一层**极淡的自上而下渐变**（`--lan-surface`→`--lan-paper`）模拟水面纵深；主区/单页卡片 `--lan-surface`；侧栏可用 `--lan-abyss` 深色"渊面"衬出页面树。
- **澜纹点睛**：`[[wikilink]]`、链接、对话气泡的强调色用 `--lan-ripple`，hover 下沉到 `--lan-deep`；侧栏激活页加一道 `--lan-ripple` 细左边框（如一道涟漪），断链用 `--lan-mist` 标灰（"水静无澜"）。
...
- **会话落盘与跨重启恢复** → 推后：v1 会话**仅内存、不调 `persist_agent_session`/`load_session`**、进程退出即清。落盘恢复（含 LRU 淘汰）以后再做。
...
| `test_web.py`（报告） | `/api/report/check|health|lint` 的 body 与同 fixture 上 `format_report(report, json_output=True)` **字节相等**（端点复用同一序列化器、不漂移；`report_json` 是 `ensure_ascii=False, indent=2`、无尾换行，故须 `Response(..., media_type="application/json")` 而非默认 `JSONResponse`）；`/graph` 触发构建并 302、`graph/graph.html` 落盘 |
...
| `test_web.py`（只读多轮 chat） | `POST /api/chat` 无 `conversation_id` → 新建会话并回传 id；**带同一 id 连发两轮 → fake agent 的 `messages` 累积、第二轮能引用第一轮上下文**；流是 `text/event-stream`、事件仅 `token`*→`done`（含 answer + conversation_id），异常 → `error`（**无工具事件**）；token 经**构造期传入的 `transport`** 流出（断言 `build_from_environment` 收到 `transport=`，**非**事后赋 `llm_text_callback`）；**省略 `--model` 时不向 `build_from_environment` 传 `model`**（断言 kwargs 无 `model` 键，绝非 `model=None`——后者会盖掉 `.env` 模型并触发 `agent.py:397` 的 `ValueError`）；fake agent 从工作线程发 `LLM_TEXT`、token 仍完整到达（验跨线程桥）；`build_from_environment` 以 `working_directory=kb`、自带 `logger`（非库内）被调用（**不**传 `permission_mode`），构造后 `permission_engine.set_mode(READ_ONLY)` **与** `tool_runner.set_readonly_mode(True)` 均被调用（缺任一断言失败），且 `guanlan-wiki` 被激活；同一会话两轮被 `asyncio.Lock` 串行；会话**不触** `raw/` 快照 / check；**不调** `persist_agent_session`/`load_session`（仅内存）；`DELETE /api/conversations/{id}` 丢弃内存对象 |
...
| **C7** | `docs: README 更新至 P4 状态 + CLAUDE.md 命令` | `README.md`、`CLAUDE.md`（补 `guanlan web` 与状态行）、本文如有微调 | `uv run pytest` 全绿；README 命令可照抄跑通 | 文档与代码一致；标注 web 为**可选叠加层**、需 `guanlan[web]` |
...
- [ ] **嵌入只碰文档化面**：`build_from_environment`/`.arun`/**构造期 `transport`**（`build_compat_transport`/`SdkTransport`）；token 流靠该 transport（`EventType.LLM_TEXT`），**不**事后改 `self.agent.llm_text_callback`（只读存档、不会流）；显式激活 `guanlan-wiki`；自带 `logger`（不落 `<wd>/agentao.log`）；**仅内存、不调** `persist/load_session`。
