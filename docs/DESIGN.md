...
1. **个人版（先做）** —— 以 **Skills** 的形式接入 Agent，纯文件（Markdown），通过 **Agentao CLI + 极薄的 `guanlan` 包装器** 跑 ingest/query。这是 MVP，也是验证理念的最小闭环，**不依赖任何后端服务**。
   ...
   - Web UI 是 MVP 之后（P4）叠加的**可选宿主层**，不改变"纯文件"的本质——它只是 ingest/query 的另一个入口。无 Web UI 时，整套东西用 CLI 一样跑通。
...
> 维护引擎在**观澜项目仓库**里（不复制进用户库）：`skills/guanlan-wiki/`（`SKILL.md` + `references/conventions.md` + `scripts/*.py`，如 `check.py`/`health.py`/`build_graph.py`）。两种运行方式，**各只对应一条路径，不混用**：
>
...
│   └── scripts/               # check.py / health.py / build_graph.py …
...
- **`guanlan-wiki/` skill（观澜安装包提供，不在用户库内）** —— **维护引擎**：`SKILL.md`（工作流）+ `references/conventions.md`（通用默认约定：页面类型/frontmatter/命名/index·log 格式/模板，按需载入，`SCHEMA.md` 可覆盖）+ `scripts/*.py`。wiki-无关、可复用、可独立升级。
...
| **基础校验** | ingest / query backfill 收尾**强制**运行 / `guanlan check` | 独立 `guanlan check` 只做 frontmatter + 断链；ingest / `--backfill` 的 wrapper 门禁在此之上**额外**做 `raw/` 调用前后快照比对（独立 check 无"调用前"基线，故不含此项）。**零 LLM**，见 §4.7 | P2 |
| **health** | `health` | 结构检查：空页/桩页、index 与磁盘同步、log 覆盖。**零 LLM** | P3 |
| **lint** | `lint` | **只做结构 lint**：孤儿页、断链、稀疏页、缺失实体页。**零 LLM** | P3 |
| **graph** | `build graph` | **只做确定性**：解析 `[[wikilink]]`→边，输出 `graph.json` + 自包含 `graph.html`。**零 LLM** | P3 |

> LLM 只用于 **ingest / query**（单轮优先、必要时分步）；其余工作流全部零 LLM。
>
> **MVP ingest 只承诺 `.md`**，多格式推到 **P5**。**语义 lint**（矛盾/过期/资料缺口，需 LLM）、**graph 增强**（LLM 推断边、Louvain、增量缓存）、**图感知 lint** 均**不进 P3**，作为后续优化项（见 §8）。先用确定性的 health + 结构 lint + wikilink graph 快速闭环，避免过早引入 LLM 成本。
...
- **矛盾标记（卖点核心，必须有确定格式）**：ingest 时若 LLM 发现新资料与既有页冲突，**就地**在相关 entity/concept 页维护一节 `## ⚠️ 矛盾与存疑`，每条一行：
  ...
  并向 `log.md` 追加一条（有无矛盾以 `## ⚠️ 矛盾与存疑` 标题为准，lint/health 直接 grep 该标题统计，不另存 frontmatter 布尔）。**ingest 只做"发现即就地标记"**；系统性复检与状态流转（open→resolved、过期论断、跨页矛盾发现）属语义 lint，P3 之后再加（见 §4.4 注 / §8）。

### 4.6 工具（观澜安装包内 `guanlan-wiki` skill 的 scripts，Python 3.12）

- **确定性脚本（零 LLM）**：`check.py`（**P2 基础校验**：frontmatter + 断链；`raw/` 不变性由 wrapper 快照比对兜底，见 §4.7，不在脚本里巡检）、`health.py`（**P3** 结构检查：空页/桩页、index 同步、log 覆盖）、`build_graph.py`（**P3** 确定性 wikilink graph）。P2 只交付 `check.py`，避免把 P3 的 health/lint 提前塞进 P2。
- **ingest / query 的 LLM 部分不写成脚本** —— 由 `guanlan-wiki` skill 驱动 Agentao（`Agentao.chat()` / LLMClient）完成。脚本只做确定性工作。（结构 lint 为零 LLM 的结构检查，属 P3；语义 lint 以后再加，见 §8。）
...
- **`guanlan ingest <raw-file>` 与 `guanlan query --backfill` 是唯二写入口。** wrapper 调用 Agentao + `guanlan-wiki` skill 让 Agent 写 `wiki/`，收尾强制运行 `check.py`：通过则保留本次 wiki 改动；失败则报告失败项，**`wiki/` 改动原样留在磁盘供检查**，由用户手动修正或删除（单用户 MVP 可接受）。需要一键撤销者，可自行把 `wiki/` 纳入 git（可选，非 MVP 机制）。
- **`guanlan query` 默认只读。** 普通 query 不写 wiki，只输出答案；只有显式 `--backfill` 才允许写 `wiki/syntheses/`，并与 ingest 使用同一套门禁。
- **`raw/` 不变性由 wrapper 确定性快照兜底，不押在权限配置上。** `ingest` / `--backfill` 在调用 Agentao **前**对 `raw/` 取一次快照（文件名 + size + mtime，必要时 SHA256），收尾再比对；任何增、删、改、重命名都判门禁失败并报告。此检查自包含、可测，不依赖 Agentao 权限规则的实参键名细节，也能拦住 shell 里 `mv`/`cp`/`rm`/`python` 等绕过 `write_file` 的写入——而单条用户级 deny 规则做不到这点。`AGENTAO.md` 仍声明"永不改 `raw/`"作为 Agent 软约束；用户若愿意，也可在 `~/.agentao/permissions.json` 额外加 deny 规则作纵深防御，但 MVP 的**硬约束是 wrapper 快照**，不是手工权限配置。
- **门禁内容保持确定性且最小。** P2 门禁 = `check.py`（frontmatter + wikilink 断链）+ `raw/` 快照比对两部分；均零 LLM、不做语义判断。
...
Agentao 是 Python 的**受治理 Agent 运行时**（约束 / 连接 / 可观测）。MVP 通过 `guanlan` wrapper 调用 Agentao CLI/运行时来执行 ingest/query；P4 起可**当库嵌入**。

### 5.1 MVP（CLI）集成

MVP 使用 Agentao 的文件约定 + skill discovery，并增加一个只负责编排和门禁的 `guanlan` wrapper：

- **`guanlan-wiki` skill** —— `description` 触发；工作流指令里让 agent 直接 shell 调用确定性脚本。发现方式（Agentao skill 路径见 `agentao/skills/manager.py`），开发与外部各一条、不混用：
  - **开发期**：`working_directory` = 观澜仓库根，skill 在 `skills/guanlan-wiki/`，命中 repo-root 路径（`<wd>/skills/`，优先级最高）自动发现，**免安装**；仓库根兼作 sample wiki（见 §4.2）。
  - **外部真实 wiki**：装入**全局 `~/.agentao/skills/guanlan-wiki/`**（cwd 无关），以用户库为 `working_directory`。用户库保持最小、不含 skill。
- **`guanlan` wrapper** —— `init` 生成模板；`ingest / query --backfill` 收尾强制门禁：跑 `check.py`（frontmatter + 断链）并比对调用前后的 `raw/` 快照，两项全过才算成功；`query` 默认只读。
...
P4 起把 Agentao 当库嵌入（`Agentao(...)`），启用 Tool 注入（包装已有脚本）、Web 流式、仓库内权限固化等。Web UI 与嵌入式细节**另见附录 A**，不属于 MVP。
...
| **P2 — 最小闭环** | wrapper + ingest + query + 基础校验 | `guanlan ingest/query`；`.md` ingest；source/entity/concept 页生成；wikilink + frontmatter + 断链校验 + `raw/` 快照比对（见 §4.7）；query 带引用；纯 CLI |
| **P3 — 健康与图谱** | health + 结构 lint + 确定性 graph | `health.py`、结构 lint（孤儿/断链/稀疏/缺失实体，零 LLM）、`build_graph.py`（仅确定性 wikilink graph）+ `graph.html` |
| **P4 — Web UI（可选）** | 进程内嵌 Agentao 的 Web 宿主 | 复用同一文件库的图形入口；技术细节见**附录 A** |
...
- **skill 分发（非阻塞）**：开发期用观澜仓库 `skills/guanlan-wiki/`（repo-root 自动发现，免安装，见 §5.1）；分发期再装入全局 `~/.agentao/skills/`，分发方式（pip/打包/一键安装）后续优化，不影响 MVP 运行。
...
- **graph 增强（P3 之后）**：在确定性 wikilink graph 之上是否加 LLM 推断边（`INFERRED`/`AMBIGUOUS`）、Louvain 社区检测、图感知 lint、SHA256 增量缓存——成本/收益待评估，需可关闭与采样策略。
