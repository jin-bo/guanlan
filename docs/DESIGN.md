...
- **推到 P5（已实现）**：多格式转换 adapter（统一接口 `convert(file) -> md`）。**P5.2 `guanlan convert` 已落地**——改为复用既有 `pdf-to-markdown` skill（MinerU→marker→pypdf 分层兜底）当后端引擎（非 markitdown），把多格式转成 `raw/*.md` + `origin` provenance，复用 `.md` 单格式 ingest 不动；脚本零 LLM、无新依赖，见 [`P5.2-多格式摄入.md`](P5.2-多格式摄入.md)。MVP ingest 仍只吃 `.md`。
...
- **门禁内容保持确定性且最小。** P2 门禁 = `guanlan check`（frontmatter + wikilink 断链 + `sources` 解析）+ `raw/` 快照比对两部分；均零 LLM、不做语义判断。**写门禁额外保证既有页源不回退**（半阶段 **P2.1**，已实现）：复用同一处 `raw/` 写前快照再取一份每页指纹，写后确定性比对——既有页 `sources` 只增不减（覆盖丢源 → `sources.dropped` 阻断 + 回喂自愈），正文腰斩级骤缩出非阻断 `body.shrank` 警告；合并逻辑仍归 Agent（工具只验不变量），细化见 [`P2.1-摄入写入纪律.md`](P2.1-摄入写入纪律.md)。
...
| **P2 — 最小闭环** | wrapper + ingest + query + 基础校验 | `guanlan ingest/query/check`；`.md` ingest；source/entity/concept 页生成；wikilink + frontmatter + `sources` + 断链校验 + `raw/` 快照比对（见 §4.7）；query 带引用；纯 CLI。**实现级细化见 [`P2-最小闭环.md`](P2-最小闭环.md)**；半阶段 **P2.1**（已实现）写门禁加既有页源不回退 `sources.dropped`（阻断+自愈）+ 正文骤缩 `body.shrank`（警告），见 [`P2.1-摄入写入纪律.md`](P2.1-摄入写入纪律.md) |
| **P3 — 健康与图谱** | health + 结构 lint + 确定性 graph | `guanlan health/lint/graph`：结构健康检查（桩页/index 同步）、结构 lint（孤儿/断链/缺失实体，零 LLM）、确定性 wikilink graph + 自包含 `graph.html`。**实现级细化见 [`P3-健康与图谱.md`](P3-健康与图谱.md)**；半阶段 P3.1–P3.6 已落（别名/heal/规范标题/reindex；**P3.5 图谱分析** 确定性 Louvain 社区 + hub/bridge/silo 拓扑 lint、**P3.6 图论桥与割点** 确定性迭代 Tarjan 割边/割点，均零 LLM）见 [`P3.5-图谱分析.md`](P3.5-图谱分析.md)、[`P3.6-图论桥与割点.md`](P3.6-图论桥与割点.md)；**P3.8 链接归一**（wikilink 解析期 fold 消变体 `_`↔`-`/NFKC、零正文改写、零 LLM；**已实现**）见 [`P3.8-链接归一.md`](P3.8-链接归一.md)；**P3.10 页型↔目录一致性**（`health` 两类 schema 漂移 advisory：`type_dir_mismatch`/`uncharted_page`、不解析 SCHEMA.md、零 LLM；**已实现**）见 [`P3.10-页型目录一致性.md`](P3.10-页型目录一致性.md)；**P3.7 语义审计**（新开 `audit` 命令：确定性粗筛→LLM 门禁内复核两层、复刻 heal，首次把 LLM 引入维护线；拱心石 `raw_digest` provenance 由 wrapper 门禁后直写 source 页（决策P3.7-3a / Option A）；**决策齐备、待实现**）见 [`P3.7-语义审计.md`](P3.7-语义审计.md) |
| **P4 — 可选宿主层（Web + MCP + IM）** | 复用同一文件库的本地宿主入口（浏览器 / Agent / 聊天窗口） | `guanlan web`：浏览 wiki + 触发 `ingest` + **与 agent 单轮/多轮问答** + 跑 check/health/lint + 看 graph；FastAPI/uvicorn 薄表现层，写作业走子进程、所有问答走只读进程内嵌入（默认只读、token 流式）。**实现级细化见 [`P4-Web宿主.md`](P4-Web宿主.md)**；半阶段 P4.1–P4.9 已落。**P4.10 MCP 宿主**（第二种传输：FastMCP **只读 stdio**，把**既有只读核心**暴露给任意 Agent——七个只读工具复用 P5.0/P5.1 `search_result_dict` 检索单一归口、新抽 `report_dict` 体检/lint 信封、CLI query 只读路径，`page`/`path` 统一为相对库根带 `wiki/` 前缀同口径；**不碰 `chat.py`**（客户端自持对话、零服务端会话）、镜像 P4.9 reader **KB 零写**、stdout 即 JSON-RPC 帧须洁净；**已实现**：`guanlan mcp` + `guanlan/mcp/` 子包，extra `guanlan-wiki[mcp]`）见 [`P4.10-MCP宿主.md`](P4.10-MCP宿主.md)；**P4.13 Web mermaid 渲染**（纯前端表现层增强：把 wiki 页 / 对话答案 / 暂存区预览里用户或 LLM 手写的 ` ```mermaid ` 围栏块在浏览器内渲染成图——**服务端零改**（`fenced_code` 已把图源保留为 `code.language-mermaid` 转义文本，`render.py` 字节不动、`_EscapeHtmlExtension` 转义姿态不破）、前端 **vendored** mermaid 运行时（非 CDN、**自包含 UMD 单文件** `mermaid.min.js` 经注入 `<script>` 懒加载，归类「内容渲染器」与已 vendored 的 `markdown` 同位阶、对决策P4-3 有界破例）、`securityLevel:'strict'`+`htmlLabels:false` 安全闸（「内容→客户端 HTML」首次破例的信任边界，不承诺产物绝对可信、CSP 列独立加固）、优雅降级保留源码（markdown 仍唯一事实来源）；`enhanceMermaid` 挂 `paintPage`/staging `paint()` 重绘函数内令切语言/切预览模式重增强；**已实现**：`static/vendor/mermaid.min.js` + `static/mermaid_enhance.js` + 四注入点接线，真浏览器冒烟已过）见 [`P4.13-Web-mermaid渲染.md`](P4.13-Web-mermaid渲染.md)；**P4.14 Web 数学/化学/代码渲染**（承 P4.13 同骨架：把 `$…$`/`$$…$$`/`\(…\)`/`\[…\]` 数学、mhchem `$\ce{}$` 化学、` ```X ` 代码块在浏览器内渲染成富呈现——**服务端零改**（代码块同 mermaid 用 `code.language-X` 转义文本载体；数学**无围栏载体**、作普通文本穿过、落成转义文本节点，**新机制** KaTeX `renderMathInElement` 遍历文本节点排版，`render.py` 字节不动、转义姿态不破）、前端 **vendored** KaTeX 0.16.22 家族（`katex`+`mhchem`+`auto-render`+css+20 字体，归「内容渲染器」对决策P4-3 有界破例）+ highlight.js 11.10.0 common、**安全闸** KaTeX `trust:false`（默认即安全的白名单铰链，禁 `\href`/`\html*`；**取 KaTeX 而否 MathJax** 因后者需脆弱的全量包减法黑名单）+ 不共享 `macros`（跨容器宏隔离）+ highlight v11 转义契约，不承诺产物绝对可信、CSP 列独立加固、失败保留源码；P4.13 单钩子泛化为 `enhanceContent` 编排器、四注入点改接；**已实现**：`static/vendor/katex/` + `vendor/highlight/` + `math_enhance.js`/`code_enhance.js`/`content_enhance.js` + 四注入点接线 + `app.css` hljs 昼澜主题，真浏览器冒烟 16/16 已过含注入/宏不跨容器/懒加载运行时验证）见 [`P4.14-Web数学化学代码渲染.md`](P4.14-Web数学化学代码渲染.md)；**P4.15 Web 工具确认 / `ask_user` 人在环**（把 P4.5 可写会话里**静默自动放行**的 ASK 决策补成**浏览器内人确认**：workspace-write 下带操作符 shell / `requires_confirmation` 工具经 SSE 弹 `confirm_request`、阻塞 turn 等用户点 允许 / 本会话起自动放行 / 拒绝、回传布尔；模型 `ask_user` 同机制弹 `ask_request`、收答案串。确认/提问回调在 arun 的 executor 线程阻塞、`call_soon_threadsafe` 推帧 + `queue.Queue` 收应答、端点瞬返不取写锁；**超时**（`--confirm-timeout`，默认 120s）+ 停止 / 断线一律**默认拒绝**兜「人走开→写锁永占」（断线先 `request_stop` trip 令牌再取消外层）；气泡②「本会话起自动放行」取 CLI 第 2 项**安全版**——只切本会话 `confirm_mode=auto`、姿态仍 workspace-write、层①②③ 不动、`raw/`/`AGENTAO.md` 仍硬只读、**非 full-access**、可逆；与层①②③**正交**：**「人点允许 ≠ 绕过 immutable」**，确认只决定 ASK 工具跑不跑、不开新写路径；pending 记 `kind` 跨类拒（`/answer` 打到 confirm pending → 409，杜绝非空串被当 truthy `True`）；`--confirm {ask,auto}` 默认 ask；**已实现**：`conversation.py` 回调 + 线程桥、`app.py` 三端点、`cli.py` 旗标、前端确认/提问气泡，`tests/test_web.py` 20 例 + `scripts/smoke_p415.py` 真浏览器冒烟 9/9）见 [`P4.15-Web工具确认.md`](P4.15-Web工具确认.md)；**P4.16 Web 长任务目标与自动续跑 `/goal`**（在浏览器对话框设一个长任务目标，由**宿主拥有**的 `while goal.is_active` 外层循环反复驱动 `Conversation.turn()`、每轮注入续跑 prompt、host 记时间/轮数预算、撞预算跑恰好一次收尾轮；agent 仅经注入的 `update_goal` 工具自报 `complete`/`blocked`、用户经 `/goal [<目标>\|show\|pause\|resume\|edit\|budget\|clear]` 控制。**最大化复用 agentao 已落地原语** `cli.goal_state.GoalState`/`cli.duration.parse_duration`/`tools.goal.UpdateGoalTool`（依赖上提至 `agentao[cli]>=0.4.13`），guanlan 只建 Web 编排层：per-conversation `.agentao/goals/<id>.json` sidecar（`os.replace` 原子写）、async `run_goal` 续跑循环（每内层轮 `ensure_future + asyncio.shield`、`_goal_lock`+`_GoalProxy` 串行 GoalState、`_goal_io_lock` 串行落盘 + `_goal_deleted` 防删后 resurrect）、`POST /api/chat/{id}/goal{,/budget,/pause,/resume,/edit,/clear,/run}` 端点（`/run` 走 SSE 续跑流 + 轮边界帧 + 多气泡 + goal 横幅）、前端 `/goal` 斜杠支线、`guanlan web --goal {on,off}`（reader 强制关）；**不放宽任何边界**（goal 每内层可写轮照走 P4.5 层①②③+check+撤销+单写者写锁逐轮取放、与 P4.15 confirm/ask 自动组合、进程内至多一个活跃可写 goal、停止断整 goal/断线收 paused/重启 active 显示可恢复不自动续）、`--turns`（外层续跑轮）⊥ `max_iterations`（内层工具循环）、不走 `force_continue`、零 agentao 内核改动；设计三轮 + 实现后 xhigh 工作流 + 多轮 codex 评审收敛至零 finding（`_in_goal`/预订零泄漏、中断轮墙钟计入、删会话竞态串行、冷会话懒恢复、孤儿 sidecar 可清）；**已实现**：`web/goal_io.py` + `conversation.py`/`conversation_store.py`/`app.py`/`server.py`/`cli.py` + `static/{chat.js,i18n.js,app.css}`，`tests/test_web.py` goal 套件 + 真浏览器冒烟 `scripts/smoke_p416.py` 5/5）见 [`P4.16-Web目标续跑.md`](P4.16-Web目标续跑.md)；**P4.17 MCP 远程传输**（给 P4.10 stdio-only MCP 宿主补第二种传输：官方 **Streamable HTTP**（spec 2025-03-26 起、取代 legacy HTTP+SSE），`serve_mcp` 加 `--transport {stdio,http}` 分支暴露**同一套只读工具**（`tools.py` 零改、`build_mcp` 仅加 `allow_ask` 注册门；传输/绑定/鉴权接线全在 `serve_mcp`）；**安全默认**——默认仍绑 `127.0.0.1`（沿用决策P4-4）、非环回强制 bearer token（最小常量时间比对、不引 OAuth）否则拒启、`ask` over http 默认关（`allow_ask=(transport=='stdio' or --allow-ask)`：stdio 永远七工具、http 默认六、`--allow-ask` 才七，避成本/DoS 放大）、无状态 `stateless_http` + DNS-rebinding 防护、TLS 外置；完整 OAuth（MCP 2025-06-18）/多租户 source 级 scoping 显式推 E2；已实测装的 mcp SDK 支持 `transport="streamable-http"` 且自带 uvicorn/starlette 故 `[mcp]` extra 与 SDK 下界均零依赖变更；编号辨析：属 P4 宿主家族兄弟半阶段（非 P4.10.1 补漏 / 非新里程碑 / 非直接 E2）；**已实现**：`guanlan mcp --transport http` + `guanlan/mcp/server.py` http 分支（`_http_security`/`_BearerTokenMiddleware`/`_build_http_app`/`_serve_http`）+ `cli.py` 六旗标，`tests/test_mcp.py` P4.17 套件含真 uvicorn 端到端往返）见 [`P4.17-MCP远程传输.md`](P4.17-MCP远程传输.md)；**P4.18 MCP 2.0 迁移**（底座从官方 SDK v1 `mcp.server.fastmcp.FastMCP` 迁到 **v2 `mcp.server.mcpserver.MCPServer`**，对齐协议修订 **`2026-07-28`**：**等价迁移**——命令契约/工具集/只读契约/退出码逐字保留（决策P4.18-1），用户可见变化恰好两处：**依赖契约**——`[mcp]` extra 硬切 `mcp>=2,<3`（v1.x 转只收安全修复的维护态、v2 删掉整个 `fastmcp` 包无 alias，故不做双大版本 shim，决策P4.18-2），**并连带把核心依赖 `agentao[cli]` 下限抬到 `>=0.4.17`**——agentao 的 mcp 1.x/2.x 探针兼容层是 0.4.17 才有的，不抬则「pip 认为下限已满足、只把 mcp 抬到 2.x」会让 `guanlan mcp` 照常工作而 ingest/query/Web 问答全死在 agentao 侧 Tool schema 上（决策P4.18-10）与 `initialize.serverInfo.version` 改报 guanlan 自身版本（v1 曾回所装 SDK 版本、v2 默认空串，决策P4.18-12）；http 姿态从 `mcp.settings` 后置赋值改走 `streamable_http_app(...)` kwargs（v2 已无这些 Settings 字段，决策P4.18-4）；**协议红利**——无状态核心/`server/discover`/标准错误码白拿（我们本就 `stateless_http` + 零服务端会话），`Mcp-Method`/`Mcp-Name` 路由头留给反代限流（服务端不强制），**cache hints 与 tasks 扩展显式不做**（前者只覆盖 list/read 类方法、救不了大 payload；后者 SDK 2.0.0 无现成实现且与无状态冲突，决策P4.18-8）；**旧客户端不掉线**已在 stdio/http 双传输实测（`initialize` 2025-06-18 正常协商，客户端首帧决定 era、同一连接不得混 era，决策P4.18-7）；测试面：in-memory 改 `Client(mcp, mode="legacy")` 保住 v1 的真 JSON-RPC 覆盖（v2 默认 mode 走 `DirectDispatcher` 无帧，决策P4.18-11）+ 新增默认 mode 平价用例 + **真 stdio 子进程逐帧洁净断言**（决策P4.18-6，此前只有 in-memory 推断）；**已实现**：`guanlan/mcp/{server,tools,__init__}.py` + `cli.py` 版本口径提示 + `pyproject.toml` extra，`tests/test_mcp.py` 63 例全绿）见 [`P4.18-MCP2.0迁移.md`](P4.18-MCP2.0迁移.md)；**P4.19 Web MCP 配置诊断**（**方向与 P4.10/P4.17/P4.18 相反**：那三个是观澜作 MCP **服务端**，本半阶段诊断的是观澜作 MCP **客户端**被注入的外部 server——agentao 的 `build_from_environment` 在调用方不传 `mcp_registry=` 时**默认注入** `FileBackedMCPRegistry`（读 `<kb>/.agentao/mcp.json` + `~/.agentao/mcp.json`），而观澜的 CLI 子进程与 Web 嵌入**都吃这个默认**，故外部 MCP 工具早已在注入 ingest/query/heal/audit/Web 问答，用户却在观澜里看不到任何痕迹；本半阶段只补这份**可见性**——`GET /api/mcp` 如实展示配置 + `POST /api/mcp/check` **用户显式点一次**连接检查，**零 LLM、零策略、零管理、不写 KB、不执行任何外部工具**（只 `initialize` + `tools/list`），**不改变注入行为**；措辞按决策P4.19-8 钉死为「当前磁盘配置的解析结果、已有会话需新建后生效」（MCP 是会话构造期一次性加载）；两条轨——生效集合取上游 `load_mcp_config` 归一结果、诊断另读原文（上游对坏 JSON **静默返回 `{}`**，只信它就只会显示「没有配置」而非「配置写坏了」）；脱敏覆盖到**连接错误文本**（`NonMcpEndpointError` 会把完整 URL 写进消息，只脱敏端点等于没脱敏）；单飞 + 并发 409 不排队、不设端点级墙钟（`to_thread` 线程不可取消，沿用上游 per-server startup timeout）、reader 下两端点**不注册**（理由是**有外部副作用**而非写 KB）、缺 `[mcp]` extra 回 501 + 安装指引；**已实现**：`web/mcpdiag.py` + `app.py` 两端点 + `static/{reports.js,index.html,i18n.js,chat.js,app.css}`，`tests/test_web_mcpdiag.py` 16 例含真 stdio 端到端（拿 `guanlan mcp` 当被测上游）与两条脱敏反向用例）见 [`P4.19-Web-MCP诊断.md`](P4.19-Web-MCP诊断.md)；**P4.20 Web flint 数据图表渲染**（承 P4.13/P4.14 同骨架的**第三类内容渲染器**：把 wiki 页 / 对话答案里的 ` ```flint ` 围栏块（[flint](https://github.com/microsoft/flint-chart) 语义图表规格 JSON）在浏览器内**编译**成 ECharts option 并渲染成 SVG 图表（37 种图型）——**服务端零改**（`render.py` 字节不动、转义姿态不破）、前端 **vendored** `flint-chart@0.4.1`（自包含单文件 **ESM**，经动态 `import()` 加载，本仓唯一走 ESM 的 vendored 资产，**仍无 npm、无构建**）+ `echarts@6.1.0`（自包含 UMD），P4.14 的 `enhanceContent` 编排器令**七个注入点零改动**；**上游触发条件**是观澜经 MCP 接入用户的结构化数据库后有了可视化需求——Agent 取数 → 把结果行写成数据**整体内联**的块 → 浏览器成图，**拒 `data.url`、渲染器永不出网**，故图 100% 可从那段 markdown 重建（原则 1 在本相位的形态）、`check`/`graph`/`search` 原样复用，图是**快照**不做渲染时查库/自动刷新/交互下钻；**安全闸**入口白名单（只五个顶层键）+ flint **生成式**编译（按模板生成、不 merge 用户私货，三条走私路径实测零残留）+ **键域**出口断言（不扫值域——合法数据里就有 URL 串）+ `renderer:'svg'` + `tooltip.renderMode:'richText'`（tooltip 不走 HTML 通道），**输入是纯 JSON、没有函数**故 ECharts 回调注入面天然不存在（这是选 flint 而非让模型直接写 ECharts option 的核心安全理由）；**资源闸**（结构化库接入独有：输入是查询结果而非人写的短文本）块源 64 K 字符 / 1000 行 / 画布不过大，超限**不渲染、保留源码 + 徽标、不静默截断**，且 flint **自己**超预算整批丢类别时同样拒渲（读 `_warnings`）；**总原则「能让 flint 自己算的绝不在事后改」**——按栏宽收窄走 `canvasSize` **重编译**而非事后改画布（flint 把 `legend.left` 等烤成绝对像素，缩画布会把图例挤出视口）；ECharts 是**前端第一个有状态渲染器**，须显式 `dispose` 脱树实例；配套**独立**作者 skill `flint-chart-author`（模型不会自发写 flint，没有作者侧供给渲染器就是「通了电没有灯」；作图与 wiki 维护正交，故不进 `guanlan-wiki`，沿 `pdf-to-markdown` 先例）；**已实现**：`static/vendor/flint/echarts.js` + `vendor/echarts.min.js` + `static/flint_enhance.js` + 编排器一行接线 + `skills/flint-chart-author/`，`tests/test_web.py` 16 例 + 真浏览器冒烟 `scripts/smoke_p420.py` 18/18（含 **37 图型全集扫描**——闸子不只要「拦得住」还要「不乱拒」，该扫描一次揪出四例误杀，已设为常驻场景 + 升级复核项））见 [`P4.20-Web-flint图表渲染.md`](P4.20-Web-flint图表渲染.md)；**P4.21 IM 宿主 `guanlan im`**（P4 的**第三种传输**：Web 是浏览器里的人、MCP 是 Agent 客户端，IM 是**聊天窗口里的人**——同一套只读会话逻辑 `Conversation.turn()`、同一零写契约、同一零业务智能。形态是「**一个平台无关的核心 + 若干薄适配器**」而非「某一个平台的宿主」：核心永不写 `if platform ==`，一切分派只读 `AdapterCaps`；v1 两个适配器**恰好互补**（个人微信不能编辑、只能整条发；飞书能原地改写）——核心一行未改即驱动两者，这就是分界的验收标准。**新增的信任线是身份与受众面**（P4.11 管提示词注入、P4.17 管网络传输，两条都不覆盖它：一个零监听端口的进程，照样能经**出站**长连把整库答案送进一个 200 人的群），故默认 deny + 空白名单拒启，群聊放行 = `chat ∈ allow_chats` **AND** (`allow_all` **OR** `user ∈ allow_users`) **AND** `mentioned_me`（`--allow-all-users` **只**旁路用户名单、**永不**旁路群名单）；身份可信度**分档**——企业系强档、个人微信弱档（iLink 标识不可枚举、不可反查），弱档只适合小圈子，且新增 `guanlan im-identify`（零 LLM、**绝不回复**、限时、把完整 ID 打到终端）解开「白名单要 ID、拿 ID 要看日志、日志脱敏」的死锁。会话 TTL 与腾容量**归宿主自持的 `SessionRegistry` 独占**（`store.idle_ttl=None`）——已核实 `store.get()` 命中即刷新 `last_active`、回收只在 `create`/`restore` 触发、顶满是**抛错而非逐出**，两套 TTL 并存反而造出一个谁都不负责的空档；过期语义靠**墓碑**（`(None, last_seen)`）保住。停机纪律：收流 task 可 `cancel()`（只阻塞在 I/O），问答 turn **绝不可以**——`arun` 收到 `CancelledError` 是置令牌后**立刻 re-raise、不等线程**，取消它是**把「还没停」伪装成「已经停了」**；故只 `request_stop()` 并**无限期**等真实收尾，30 秒只打 WARNING。**并如实声明优雅停机没有绝对时间上界**——模型侧的 `read=600` 是**每次读**的空闲上限而非整轮墙钟，外部 MCP 那条链上出站写入 / legacy SSE 传输进入 / stdio 传输进入 / 断连清理**都没有人管**（`--mcp-request-timeout` 只收住「写完之后等 `tools/call` 回话」那一段），**唯一的绝对保证是第二次中断信号的 `os._exit(5)`**（那条路上连 `logging` 都不许碰）；总 deadline 记为 agentao 上游诉求。长答案契约是**「不静默截断」而非「绝不截断」**（三条原要求数学上不可同时满足），末片是显式截断告示且**不得称「完整版」**——IM 会话 `persist=False`，被舍弃的内容没存在任何地方；**已实现**：`guanlan/im/` 子包（contract/intake/session/reply/delivery/state/server/mcp_bound + 两个适配器）+ `guanlan im` / `im-login` / `im-identify` 三个子命令 + `[im-weixin]`/`[im-feishu]` 两个 extra，`tests/test_im{,_weixin,_feishu}.py` 136 例）见 [`P4.21-IM宿主.md`](P4.21-IM宿主.md)；**P4.22 IM 可点引用**（给 P4.21 补最后一公里：答案里的 `[[某页]]` 在飞书里**可以点**、点完机器人把那一页发回来——**不是新宿主、不开监听端口、不引入任何写路径**，回调走已建好的那条 WS 长连故决策P4.21-21 无监听面继续成立、`/page` 只读盘故决策P4.21-8 KB 零字节写继续成立。三件事：**① `/page <名字>`**（所有平台、零 LLM，复用 P3.8 单一 owner 归口，故精确 stem / 别名（P3.1）/ 安全 fold variant（P3.8）三种写法都认——IM 侧另写一套按名查找就是给这个系统开第五个口径；未命中降级为 top-3 检索；截断告示**不附 Web 入口**，因为 Web 至今没有按名开页的路由、给出去仍是死链）；**② 可点动作的平台无关表达**（`Action` 携带的是**核心生成的命令文本**而非"页面名"或"动作类型"，适配器只把这句话塞进按钮、回调时原样取回——**它不知道 `/page` 是什么**；`AdapterCaps` 加 `supports_actions`/`max_actions`，与 `typing()` 完全对称，个人微信恒关、一个字不变）；**③ 点击 = 一条合成的入站消息**（枢纽：回调经 `adapter.inbound()` **原路**回到既有流水线，五道闸全复用、**`server.py` 零改**、核心零平台分支，新增动作类型时适配器同样零改）。**红线的闸在接收端而非构造端**——`InboundMessage` 新增事实字段 `origin`，核心据此把合成消息能跑的命令收窄到 `ACTION_COMMANDS`（v1 只有 `page`，`/help`/`/new` 都是零 LLM 却**不在**其中：前者是噪音、后者会清用户上下文），因为 `classify()` 把**未知**斜杠命令当普通提问送进 LLM，只校验"以 `/` 开头"挡不住一次不该发生的 LLM 花费。**授权面**：`chat_type` 只认**真实入站消息见过**的会话（适配器自持 `(tenant, chat_id) → chat_type` 事实缓存、未命中 fail-closed）——飞书卡片可被转发，"卡片出现在哪里"不构成授权依据；`allow_chats` 一个字都不进适配器（静态断言守着），键的两半都取 `map_event` 同源字段（取错 tenant 的后果不是报错、是**间歇性"会话已过期"**）。**卡片只承载页面名、且只来自已确认送达的正文片**（`split_for` 超限会丢后缀，从完整答案抽会把正文里根本没出现过的页面名印上群内公开可见的卡片）；出按钮前先解析一次，断链引用不出按钮（不承诺一个不存在的东西）；超限**显式告示**、`[[原文]]` 始终保留故手打 `/page` 永远是退路。**CARD 帧兼容层**：`lark-oapi` 的 WS 客户端对 CARD 帧**直接 return**，而只有 EVENT 分支才走到卡片回调注册表，而"回调以哪种帧下发"证据**矛盾**（Go SDK 注释 vs 上游 issue #126 的真机 200340）——实现**不选边**，子类化 WS 客户端把 CARD 帧头就地改写成 EVENT 再委回父类，**纯粹的超集**（走 EVENT 时一次都不触发），探不到私有接缝即降级告警不拒启 + 真 SDK 形状探针，上游修好即可整段删除；**已实现**：`guanlan/im/pageview.py` + contract/reply/intake/delivery 扩展 + 飞书卡片与回调 + CARD 帧兼容层，`tests/test_im{,_feishu}.py` P4.22 套件；**帧类型 / 卡片 2.0 组件键形状 / 转发点击表现仍待真机核对**，见该文 §9）见 [`P4.22-IM可点引用.md`](P4.22-IM可点引用.md) |
| **P5 — 语料规模化与自动化（多格式 + 检索）** | 多格式摄入、检索、定时同步 | **P5.0 检索层** `guanlan search`（确定性 BM25 + CJK 2-gram，零 LLM；E1 检索升级的零基建先行片；**已实现**）见 [`P5.0-检索层.md`](P5.0-检索层.md)；**P5.1 Web 检索接入**（P5.0 内核接入长驻 Web 进程：`GET /api/search` + 只读 `guanlan_search` 宿主工具 + 长驻 `CorpusCache`；**已实现**）见 [`P5.1-Web检索接入.md`](P5.1-Web检索接入.md)；**P5.2 多格式摄入** `guanlan convert`（复用 `pdf-to-markdown` skill 分层后端把 PDF/DOCX/… 转成 `raw/*.md` + `origin`，复用 `.md` ingest 不动、复用 `rawio` 写归口，脚本零 LLM、无新依赖；补 CLI 那个洞，Web 那半 P4.6 已落；**已实现**）见 [`P5.2-多格式摄入.md`](P5.2-多格式摄入.md)；**P5.2.1 转换图片随源落盘**（P5.2 补漏半阶段：图片随转换文件一并落 `raw/images/<slug>/<slug>-N.ext`、重写 markdown 引用使 `raw/<slug>.md` 自洽，引擎无关、仍零 LLM、仍只写 `raw/`、`rawio` 一字不改；**已实现**）见 [`P5.2.1-图片落盘.md`](P5.2.1-图片落盘.md)；**P5.3 检索 backlink 重排**（P5.0 BM25 召回叠一道确定性文档先验：命中页按入链数 `1+W·ln(1+c)` 温和乘性上浮、`compute_backlinks` 复用 `build_graph` 解析口径不另起第二套、CLI/Web/chat/MCP 四处同名次、热路 `CorpusCache` 按语料签名 memo、零 LLM 零写盘、收录门槛判在 boost 之前故召回集与纯 BM25 逐页一致；**已实现**）见 [`P5.3-检索backlink重排.md`](P5.3-检索backlink重排.md)；**P5.4 检索冷启动性能**（`CorpusCache` 懒构建——首搜一次付清 `build_corpus`+`build_graph` 两笔全库扫描 ~1 分钟、压在用户关键路径上，2 次起 `(mtime_ns,size)` 署名 memo 命中即瞬时；**已实现**：候选①启动预热——长驻 Web/MCP 起服时 daemon 线程 `CorpusCache.prewarm` 把冷算移出关键路径 + 推荐(a) 按库根分桶 singleflight 构建锁令预热与并发首搜经同一把锁合流、不双重冷扫（决策P5.4-1/2，零新派生物/旗标/退出码/契约，守 P5.0 stateless 边界）；候选②profile ③拆 corpus/backlink ④backlink 增量 ⑤持久化索引[E1] 均未做）见 [`P5.4-检索冷启动性能.md`](P5.4-检索冷启动性能.md)；nightly health/lint |
...
| **E2 — 多租户权限** | 权限隔离 | source 级作用域 + 信任边界（本地全权 / 远程 scoped）。**会话分租先行**（[`P4.9-只读多会话.md`](P4.9-只读多会话.md)：只读多用户 / 能力 UUID 隔离 / 无用户管理，作 E2 前哨在 P4 半阶段落地）；只读本地 **MCP 宿主**（[`P4.10-MCP宿主.md`](P4.10-MCP宿主.md)）作"远程 / scoped MCP"前哨，其 **Streamable HTTP 传输前哨** 已实现为 **P4.17**（localhost-first + 单 bearer-token 闸、无状态 + DNS-rebinding 防护；完整 OAuth/多租户 source 级 scoping 仍属本阶段）见 [`P4.17-MCP远程传输.md`](P4.17-MCP远程传输.md)；**鉴权口径随协议演进**——P4.18 起底座为 MCP `2026-07-28`，其授权硬化（RFC 9207 `iss` 校验、DCR 废弃转 CIMD、凭据绑定 issuer）**全在 OAuth 面、仍归本阶段**，P4.18 本身不碰（见 [`P4.18-MCP2.0迁移.md`](P4.18-MCP2.0迁移.md) §5）；**source 级作用域后续**（真正的 E2，动数据模型、排 P 轨之后）；**IM 宿主（[`P4.21-IM宿主.md`](P4.21-IM宿主.md)）作身份面前哨**——它是 P4.9 会话分租模型的第二个宿主，**不动数据模型**（所有获准用户看同一个整库只读视图，隔离的只是各自的会话历史），并新引入一条正交的信任线：**提问者是谁、答案发给谁看见**；且身份可信度**分档**（企业系强档 / 个人微信弱档，弱档 ID 不可枚举不可反查、不得冒充强档）。source 级作用域仍属本阶段 |
...
- **CJK 检索质量（按需升级，不预先加机制）**：MVP 用 `index.md` + 2-gram 粗召回，召回不中时 Agent 扫目录或请用户补关键词兜底；后续仅在漏召回确有实证时再评估 aliases 或轻量检索，分词不优先（逐级增强备选见 `docs/backlog/notes/cjk-retrieval-enhancements.md`）。**别名已落 P3.1**；**轻量检索已细化为 P5.0**（确定性 BM25 + 2-gram，整页正文召回，见 [`P5.0-检索层.md`](P5.0-检索层.md)）；**检索增强线第④档 backlink 重排已落 P5.3**（命中页按入链数做确定性文档先验温和乘性上浮、复用 `build_graph` 解析口径、零 LLM 查询期算不落派生物，见 [`P5.3-检索backlink重排.md`](P5.3-检索backlink重排.md)）；向量仍留 E1、分词仍不优先。**检索性能线**：长驻 Web 进程首搜 ~1 分钟（`CorpusCache` 懒构建、冷算两笔全库扫描）已落 **P5.4 检索冷启动性能**（**已实现**：启动预热把冷算移出用户关键路径 + singleflight 构建锁令预热/并发首搜合流，守 P5.0 stateless 边界、零契约变更），见 [`P5.4-检索冷启动性能.md`](P5.4-检索冷启动性能.md)。
- **多格式摄入（已细化为 P5.2 + P5.2.1）**：PDF/DOCX/… → `raw/*.md` 的 adapter 已落 `guanlan convert`——复用 `pdf-to-markdown` skill 的 MinerU→marker→pypdf 分层后端、宿主侧零 LLM 写 `raw/`、复用 `rawio` 写归口，见 [`P5.2-多格式摄入.md`](P5.2-多格式摄入.md)；转换抽出的图片随源落盘（`raw/images/<slug>/<slug>-N.ext`、重写引用使 `raw/<slug>.md` 自洽，引擎无关）已落 **P5.2.1**，见 [`P5.2.1-图片落盘.md`](P5.2.1-图片落盘.md)。**仍推后**：多格式**自动解析管线 / nightly 自动转换富化**（属 E3）、**高保真 OCR 多 backend 选型评估**。Web 转换接入降为 P4.6 的机会性优化（非阶段），MCP 明确不做 convert（写 `raw/` 与 P4.10 只读相悖）。
...
（**Tool 注入的事实校正**：可写多轮工作会话已在 P4.5 落地；而"Tool 注入"经 P4.19 核实**并非"尚未开始"而是"早已默认开着"**——agentao 在调用方不传 `mcp_registry=` 时默认注入 `FileBackedMCPRegistry`，读 `<kb>/.agentao/mcp.json` + `~/.agentao/mcp.json`，观澜的 CLI 子进程与 Web 嵌入都吃这个默认。**P4.19 只补可见性、不建治理**（见 [`P4.19-Web-MCP诊断.md`](P4.19-Web-MCP诊断.md)）；注入方向的准入/白名单/只读断言仍**不立相位**，已核实的源码结论存放在 [`backlog/notes/mcp客户端注入-未排期.md`](backlog/notes/mcp客户端注入-未排期.md)。）
