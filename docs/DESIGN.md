...
> 维护引擎在**观澜项目仓库**里（不复制进用户库）：`skills/guanlan-wiki/`（`SKILL.md` + `references/conventions.md`）；**确定性校验 `guanlan check` 在 `guanlan` 包内、不在 skill**（§4.6 / 决策1），P3 的 `health.py`/`build_graph.py` 落点届时再定。两种运行方式，**各只对应一条路径，不混用**：
>
...
│                               # P2 无 scripts/：guanlan check 在包内（决策1）；P3 health/graph 落点待定
...
- **确定性脚本（零 LLM）**：check（**P2 基础校验**：frontmatter + 断链 + `sources` 解析；`raw/` 不变性由 wrapper 快照比对兜底，见 §4.7，不在脚本里巡检）、`health.py`（**P3** 结构检查：空页/桩页、index 同步、log 覆盖）、`build_graph.py`（**P3** 确定性 wikilink graph）。P2 只交付 check，避免把 P3 的 health/lint 提前塞进 P2。
