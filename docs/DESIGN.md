...
- **多格式摄入（已细化为 P5.2）**：PDF/DOCX/… → `raw/*.md` 的 adapter 已落 `guanlan convert`——复用 `pdf-to-markdown` skill 的 MinerU→marker→pypdf 分层后端、宿主侧零 LLM 写 `raw/`、复用 `rawio` 写归口，见 [`P5.2-多格式摄入.md`](P5.2-多格式摄入.md)。**仍推后**：多格式**自动解析管线 / nightly 自动转换富化**（属 E3）、**高保真 OCR 多 backend 选型评估**。Web 转换接入降为 P4.6 的机会性优化（非阶段），MCP 明确不做 convert（写 `raw/` 与 P4.10 只读相悖）。
