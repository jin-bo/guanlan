...
把**高频缺失实体**（被 ≥2 页引用却无页的断链，即 `lint.missing_entity`）一次性物化成 entity 页。wrapper 已把本批**目标名 + 各自引用页清单**算好喂进 prompt（你不必自己找谁引用了它），你只需对每个目标：

1. **只读所列引用页**与 `wiki/index.md` 建上下文（别读全库；目标名已是归一键，可直接作文件名）。
2. 若上下文**足以确认其为实体**，按决定树物化：
   ...
   目标是某既有 entities//concepts/ 页的变体？（读 index.md/引用页可认出）
     ...
     否 → 目标名够规范、可直接当文件名？
            是 → A：建 entities/<目标>.md（默认最稳，[[原引用]] 经同一归一天然解析）
            否（口语/缩写，且引用上下文给出更规范全称）
                → B：建 entities/<规范名>.md + frontmatter aliases 收编原目标名
