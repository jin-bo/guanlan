<p align="center">
  <img src="https://raw.githubusercontent.com/jin-bo/guanlan/main/docs/guanlan-origin.png" alt="观澜 GuānLán" width="160">
</p>
...
答案里的 `[[引用]]` 不是死字:任何平台都能发 `/page 甲实体` 把那一页调出来(零 LLM、毫秒级);飞书上还会追发一张卡片把这些引用变成**按钮**,点一下就等于替你发了那条命令。按钮**只能开页,永远不触发任何写操作**。
...
> 💡 知识库长起来后,可让 LLM 先分析当前库的真实状态、再据此更新 `SCHEMA.md`(领域边界 / 页面类型用法 / 命名与标签约定 / 演进中的组织偏向);含可直接复用的提示词,见 **[用户指南 → 快速上手 §1.1 让 LLM 辅助更新 `SCHEMA.md`](https://github.com/jin-bo/guanlan/blob/main/docs/guide/zh/02-快速上手.md#11-让-llm-辅助更新-schemamd)**。
...
维护引擎是 `skills/guanlan-wiki/`(`SKILL.md` + `references/conventions.md` + 脚本),开发期命中 Agentao 的 repo-root skill 发现路径(`<工作目录>/skills/`),免安装。详见 [`CLAUDE.md`](https://github.com/jin-bo/guanlan/blob/main/CLAUDE.md)。
