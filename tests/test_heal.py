...
def test_heal_new_concept_page_allowed(kb: Path, capsys):
    """概念分类增强：heal 现可按实体/概念落目录，新建 concepts/ 页（算法等）解析且**不**判越界。"""
    _ref(kb, "a", "某某算法")
    _ref(kb, "b", "某某算法")
    # 算法是概念 → 建到 concepts/；[[某某算法]] 按 stem 解析，且 concepts/ 是允许写入面。
    runner = make_runner(lambda root: write_page(root, "wiki/concepts/某某算法.md", type="concept"))
    run_heal(root=kb, runner=runner, json_output=True)
    data = json.loads(capsys.readouterr().out)
