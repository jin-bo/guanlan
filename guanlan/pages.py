...
# HTML 注释，**只认独占整行的那种**：`<!--` 前只允许空白且须在行首，`-->` 后只允许空白且须到行尾；
# 中间可跨行（DOTALL）。两处收窄各挡一类漏报，都是评审实测出来的：
#
# 1. **行锚定**挡「行内标记冒充注释」。裸 `<!--.*?-->` 会让正文/行内 code 里的字面 `<!--`（写文档
#    讲注释写法时很常见）与后文任意一个 `-->` 配对，把中间的真链接整段抹掉——`check` 于是对真断链
#    退 0，写门禁静默漏报。行锚定后，行中间的标记永远不构成注释开头。
# 2. **回火** `(?!<!--|-->)` 挡「未闭合的 `<!--` 跨段吞并」。非贪婪只保证止于**第一个** `-->`，不保证
#    那个 `-->` 属于同一段注释；init 模板自带四段注释，任何一个忘了闭合的 `<!--` 都能找到配对，
#    把中间的真登记行/真断链吞掉。回火让「中间还夹着另一个 `<!--`」的匹配整体失败，于是未闭合的那段
#    原样保留、后一段真注释各归各的。
#
# 代价（有意接受）：行尾的行内注释 `正文 <!-- 备注 -->` 不再被识别，其中的链接照常参与扫描。宁可
# 多报，不可少报——门禁的漏报会让坏库一路绿灯，而误报只是吵。
#
# 行锚定**不用** `^`/`$`+MULTILINE，而用「前后不是非换行符」的环视：MULTILINE 的 `^`/`$` 只认 `\n`，
# 于是 CRLF 文本里 `-->` 与 `\n` 之间隔着 `\r`、`$` 匹配不上，整行注释**当场不再被识别**。自
# `rawio.read_text_verbatim` 上线，`reindex`/`remove` 喂进来的就是原始 CRLF / 裸 CR，这条不再是假设。
# 环视写法对 LF / CRLF / 裸 CR 一视同仁（LF 文本下与旧式逐字等价）。
# 说清边界、别夸大：`--prune` 今天**不会**因此删掉 CRLF 库的注释行——那一层由 `reindex.
# _comment_touched_lines`（宽松口径、凡沾注释标记的行一律不删）兜着。本处修的是**严格口径自己**
# 不能因行尾而失灵：`strip_html_comments` 是 `check`/`graph`/`index_md_links`/Web 渲染共用的公共
# 原语，让它对 CRLF 静默换一套含义，是给下一个消费方埋雷。
...
def strip_html_comments(text: str) -> str:
    """把**独占整行的** HTML 注释抹成等长空白——链接扫描与 Web 渲染的**共同**注释口径。
    ...
    **只认独占整行的注释**（行锚定 + 回火，理由见 `_HTML_COMMENT_RE` 上方）。这条收窄同时挡掉了
    另一类坑：行内注释若被识别，`[[Foo<!--旧名-->]]` 抹成 `[[Foo    ]]` 后会被下游的 `.strip()`
    重新粘成有效引用 `[[Foo]]`——空格并不是可靠的 token 边界，因为**消费方自己会 strip**。行锚定后
    这种紧贴内缘的注释根本不被识别，原样进扫描、照旧判断链，不会把断链伪装成通过。

    **抹成等长空白、而不是删空**：删空会把注释两侧字符粘起来（`[名]<!--注-->(entities/X.md)` 粘成
    合法 markdown 链接）。行分隔符**逐字保留**（不是统一补 `\\n`）：裸 `\\r` 断行的文本里补出的 `\\n`
    会与前一个 `\\r` 合成**一个** CRLF 边界、行数凭空少一，而 `reindex --prune`/`remove` 靠"抹注释后
    的第 i 行"与"原文第 i 行"逐位对齐剪枝。（那两条路径经 `rawio.read_text_verbatim` 读盘，喂进来的
    就是原始 CRLF / 裸 CR，**不再**有 `Path.read_text` 替本函数归一这回事。）

    于是本函数是**长度、行数、列偏移三保**的：输出与输入逐字符一一对应。

    **未闭合的 `<!--` 原样保留、不吃到文末，也不与后一段注释的 `-->` 配对**：那样会把后文真实链接
